...
class StockMarketApplication @Inject()(cc: ControllerComponents)(implicit ec: ExecutionContext) extends AbstractController(cc) {
  val MIN_REPUTATION = 60

  case class StockMarketData(airlines: JsValue, history: JsValue)

  val stockMarketLoader: CacheLoader[Int, StockMarketData] =
    new CacheLoader[Int, StockMarketData] {
      def load(cycle: Int): StockMarketData = {
        ...
        StockMarketData(airlinesJson, historyJson)
      }
    }

  val stockMarketCache: LoadingCache[Int, StockMarketData] =
    CacheBuilder.newBuilder()
      .maximumSize(2)
      .expireAfterWrite(10, TimeUnit.MINUTES)
      .build(stockMarketLoader)

  def currentCycle: Int = CycleSource.loadCycle() - 1

  def getStockMarketData() = Authenticated { implicit request =>
    request.headers.get(IF_NONE_MATCH) match {
      case Some(etag) if etag == s""""$currentCycle"""" =>
        NotModified
      case _ =>
        val cycle = currentCycle
        val data = stockMarketCache.get(cycle)
        Ok(Json.obj(
