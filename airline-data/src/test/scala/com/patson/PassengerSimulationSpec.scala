...
class PassengerSimulationSpec(_system: ActorSystem) extends TestKit(_system) with ImplicitSender
  ...
  def assignLinkConsiderationIds(linkConsiderations: List[LinkConsideration]): Unit = {
    var id = 1
  ...
  def assignLinkIds(links: List[Link]): Unit = {
    var id = 1
    links.foreach { link =>
      if (link.id == 0) {
        link.id = id
  ...
  "Find shortest route" should {
    ...
    "find route if there's a link chain to target within max hop" in {
      val links = List(
        LinkConsideration.getExplicit(Link(fromAirport, toAirportsList(0), testAirline1, LinkClassValues.getInstance(100), 10000, LinkClassValues.getInstance(10000), 0, 600, 1), 100, ECONOMY, false),
      ...
      val route = routes(toAirportsList(2))
      route.links.size shouldBe 3
      route.links shouldEqual links
    }

    "find route if there's a reverse link chain to target within max hop" in {
      val links = List(
        LinkConsideration.getExplicit(Link(toAirportsList(2), toAirportsList(1), testAirline1, LinkClassValues.getInstance(100), 10000, LinkClassValues.getInstance(10000), 0, 600, 1), 100, ECONOMY, true),
    ...
    "find a cheaper route even with connection flights (with frequent service)" in {
      val cheapLinks = List(
        LinkConsideration.getExplicit(Link(fromAirport, toAirportsList(0), testAirline1, LinkClassValues.getInstance(100), distance = 3500, LinkClassValues.getInstance(10000), 0, duration = 200, frequency = 42), 3500, ECONOMY, false),
      ...
      val route = routes(toAirportsList(2))
      route.links.size shouldBe 3
      route.links shouldEqual cheapLinks
    }

    "use direct route even though it's more expensive as connection flight is not frequent enough" in {
      val cheapLinks = List(
        LinkConsideration.getExplicit(Link(fromAirport, toAirportsList(0), testAirline1, LinkClassValues.getInstance(100), 10000, LinkClassValues.getInstance(10000), 0, duration = 200, frequency = 1), 400, ECONOMY, false),
      ...
      route.links shouldEqual List(expensiveLink)
    }
    ...
    "apply transfer discount for Traveler passengers at transfer airport" in {
      ...
      routesTraveler(toAirportsList(1)).links.size should be > 0
      routesElite(toAirportsList(1)).links.size should be > 0
    }
  }
  ...
  "IsLinkAffordable" should {
    "accept almost all route (single link) at 60% of suggested price and neutral quality and 50 loyalty" in {
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should be > 0.99
    }
    ...
    "accept some route (single link) at 70% of suggested price with 0 quality/loyalty" in {
      ...
      val (totalAcceptedRoutes, totalRoutes) = simulatePassengerAcceptance(clonedFromAirport, toAirport, newLink)
      (totalAcceptedRoutes.toDouble / totalRoutes) should (be > 0.4 and be < 0.6)
    }
    ...
    "accept most route (single link) at suggested price with neutral quality and decent loyalty" in {
      ...
      val (totalAcceptedRoutes, totalRoutes) = simulatePassengerAcceptance(clonedFromAirport, toAirport, newLink)
      (totalAcceptedRoutes.toDouble / totalRoutes) should be > 0.7
    }
    ...
    "accept some (single link) at suggested price with neutral quality and no loyalty" in {
      ...
      val (totalAcceptedRoutes, totalRoutes) = simulatePassengerAcceptance(clonedFromAirport, toAirport, newLink)
      (totalAcceptedRoutes.toDouble / totalRoutes) should (be > 0.3 and be < 0.5)
    }
    ...
    "accept some (single link) at suggested price with neutral quality and no loyalty for very low income country" in {
      ...
      val (totalAcceptedRoutes, totalRoutes) = simulatePassengerAcceptance(clonedFromAirport, toAirport, newLink)
      (totalAcceptedRoutes.toDouble / totalRoutes) should (be > 0.2 and be < 0.4)
    }
    ...
    "accept almost no link at 1.2 suggested price with 0 quality and no loyalty" in {
      ...
      val (totalAcceptedRoutes, totalRoutes) = simulatePassengerAcceptance(clonedFromAirport, toAirport, newLink)
      (totalAcceptedRoutes.toDouble / totalRoutes) should be < 0.05
    }
    ...
    "accept very few link at 1.4 x suggested price with neutral quality and decent loyalty" in {
      ...
      val (totalAcceptedRoutes, totalRoutes) = simulatePassengerAcceptance(clonedFromAirport, toAirport, newLink)
      (totalAcceptedRoutes.toDouble / totalRoutes) should (be > 0.0 and be < 0.1)
    }
    ...
    "accept very few link at 2 x suggested price with max quality and max loyalty" in {
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should (be > 0.0 and be < 0.3)
    }
    ...
    "accept no link at 2 x suggested price with no quality and no loyalty" in {
      ...
      val (totalAcceptedRoutes, _) = simulatePassengerAcceptance(clonedFromAirport, toAirport, newLink)
      totalAcceptedRoutes shouldBe 0
    }
    ...
    "accept few link at suggested price with neutral quality and decent loyalty but downgrade in class (from business to econ)" in {
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should be < 0.25
    }
    ...
    "accept some links at 2 x suggested price with neutral quality and decent loyalty for SUPERSONIC flight of Last Minute pax" in {
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should (be > 0.5 and be < 0.8)
    }
    ...
    "accept very few route with links are at 1.25 price with neutral quality and 0 loyalty" in {
      val clonedFromAirport  = fromAirport.copy()
      clonedFromAirport.initAirlineAppeals(Map(testAirline1.id -> AirlineAppeal(0)))
    ...
    "reject route with one short link at 4X suggested price at min loyalty and quality" in {
      ...
      val toAirports = List[Airport] (
        ...
        Airport("", "", "To Airport", 0, 92, "", "", "", 1, 0, 0, 0, id = 4)
      )
              ...
              case (preferredLinkClass, flightPreferenceList) =>
                flightPreferenceList.foreach { flightPreference =>
                  if (!isLoungePreference(flightPreference)) {
                    ...
                    val route = Route(linkConsiderations, linkConsiderations.foldLeft(0.0) { _ + _.cost })
                    PassengerSimulation.getRouteRejection(route, clonedFromAirport, toAirports.last, preferredLinkClass, paxType) shouldNot be (empty)
                  }
                }
            }
        }
    ...
    "accept most routes with suggested price at good quality and good loyalty" in {
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should be > 0.8
    }
    ...
    "accept most routes with suggested price at neutral quality and decent loyalty" in {
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should be > 0.6
    }
              ...
              case (preferredLinkClass, flightPreferenceList) =>
                flightPreferenceList.foreach { flightPreference =>
                  if (!isLoungePreference(flightPreference)) {
                    ...
                    val route = Route(linkConsiderations, linkConsiderations.foldLeft(0.0) { _ + _.cost })
                    PassengerSimulation.getRouteRejection(route, clonedFromAirport, toAirportsList.last, preferredLinkClass, paxType) shouldBe Some(RouteRejectionReason.TOTAL_COST)
                  }
                }
            }
        }
    ...
    "accept most route that all links are at 60% price and the total distance travel is 1.25x of the actual displacement with neutral quality" in {
      ...
      val toAirports = List[Airport] (
        ...
        Airport("", "", "To Airport", 0, 100, "", "", "", 1, 0, 0, 0, id = 3)
      )
    ...
    "reject route that all links are at suggested price and the total distance travel is 3x of the actual displacement" in {
      ...
      val toAirports = List[Airport] (
        ...
        Airport("", "", "To Airport", 0, 30, "", "", "", 1, 0, 0, 0, id = 3)
      )
              ...
              case (preferredLinkClass, flightPreferenceList) =>
                flightPreferenceList.foreach { flightPreference =>
                  if (!isLoungePreference(flightPreference)) {
                    ...
                    val route = Route(linkConsiderations, linkConsiderations.foldLeft(0.0) { _ + _.cost })
                    PassengerSimulation.getRouteRejection(route, clonedFromAirport, toAirports.last, preferredLinkClass, paxType) shouldBe Some(RouteRejectionReason.DISTANCE)
                  }
                }
            }
        }
    ...
    "reject most links at standard price if it does not fulfill lounge requirement (long flight)" in {
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should be < 0.2
    }
    ...
    "reject some links at standard price if it does not fulfill lounge requirement (short flight)" in {
      ...
      val toAirport = toAirportsList(0).copy(longitude = 10, size = 5)
      val distance = Util.calculateDistance(clonedFromAirport.latitude, clonedFromAirport.longitude, toAirport.latitude, toAirport.longitude).intValue()
      val duration = Computation.computeStandardFlightDuration(distance)
      val suggestedPrice = Pricing.computeStandardPriceForAllClass(distance, FlightCategory.DOMESTIC, PassengerType.TRAVELER, clonedFromAirport.income)
      val price = suggestedPrice
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should (be > 0.4 and be < 0.7)
    }
    ...
    "accept few links at standard price if it fulfill some lounge requirement (long flight level 1 at departing airport only)" in {
      val clonedFromAirport  = fromAirport.copy(size = 5)
      clonedFromAirport.initAirlineAppeals(Map(testAirline1.id -> AirlineAppeal(50)))
      clonedFromAirport.initLounges(List(Lounge(airline = testAirline1, allianceId = None, airport = clonedFromAirport, level = 1, status = LoungeStatus.ACTIVE, foundedCycle = 0)))
    ...
    "accept some links at standard price if it fulfill some lounge requirement (long flight level 1 at both airports)" in {
      ...
      val toAirport = toAirportsList(2).copy(size = 5)
      toAirport.initLounges(List(Lounge(airline = testAirline1, allianceId = None, airport = toAirport, level = 1, status = LoungeStatus.ACTIVE, foundedCycle = 0)))
      val distance = Util.calculateDistance(clonedFromAirport.latitude, clonedFromAirport.longitude, toAirport.latitude, toAirport.longitude).intValue()
      val duration = Computation.computeStandardFlightDuration(distance)
      val suggestedPrice = Pricing.computeStandardPriceForAllClass(distance, FlightCategory.DOMESTIC, PassengerType.TRAVELER, clonedFromAirport.income)
      val price = suggestedPrice
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should (be > 0.2 and be < 0.5)
    }

    "accept most links at standard price if it fulfill all lounge requirements (level 3 at both airports)" in {
      val clonedFromAirport  = fromAirport.copy(size = 5)
      clonedFromAirport.initAirlineAppeals(Map(testAirline1.id -> AirlineAppeal(50)))
      clonedFromAirport.initLounges(List(Lounge(airline = testAirline1, allianceId  = None, airport = clonedFromAirport, level = 3, status = LoungeStatus.ACTIVE, foundedCycle = 0)))

      val toAirport = toAirportsList(2).copy(size = 5)
      toAirport.initLounges(List(Lounge(airline = testAirline1, allianceId  = None, airport = toAirport, level = 3, status = LoungeStatus.ACTIVE, foundedCycle = 0)))
      val distance = Util.calculateDistance(clonedFromAirport.latitude, clonedFromAirport.longitude, toAirport.latitude, toAirport.longitude).intValue()
      val duration = Computation.computeStandardFlightDuration(distance)
      val suggestedPrice = Pricing.computeStandardPriceForAllClass(distance, FlightCategory.DOMESTIC, PassengerType.TRAVELER, clonedFromAirport.income)
      val price = suggestedPrice
      ...
      (totalAcceptedRoutes.toDouble / totalRoutes) should be > 0.9
    }
  ...
  "Find affordable route" should {
    "accept a route if all links are reasonable" in {
      ...
      val links = List(
        LinkConsideration.getExplicit(Link(fromAirport, toAirportsList(0), testAirline1, standardPrices(0), distances(0), LinkClassValues.getInstance(10000), 0, durations(0), 1), 100, ECONOMY, false),
      ...
      for (_ <- 0 until 10000) {
        PassengerSimulation.getRouteRejection(routes(toAirportsList(2)), fromAirport, toAirportsList(2), ECONOMY, passengerGroup.passengerType) shouldBe empty
      }
    }
  }
}