...
import com.patson.PassengerSimulation
...
object AirportRating {
  private val modelAirportPower: Long = 25_000_000 //Use static value here, HND before boost
  // By adding 'L', we ensure the multiplication is done using Longs, preventing overflow
  private val modelCountryPower: Long = 29179 * 1438239432L //Use static value here, CN before boost
  ...
  def rateAirport(airport: Airport): AirportRating = {
    ...
    val airportScalePower = if (detailedAirport.size >= 7) 15 + detailedAirport.size else detailedAirport.size //boost
    val airportSpecificBoosts = detailedAirport.iata match {
      case "LHR" => 20
      case "JFK" => 15
      case "HND" => 10
      case "AMS" => 7
    ...
    val featurePower = detailedAirport.getFeatures().map { feature =>
      feature.featureType match {
        ...
        case ELITE_CHARM => feature.strength.toDouble / 5
        case INTERNATIONAL_HUB => feature.strength.toDouble / 5
    ...
    }.sum.toInt + airportSpecificBoosts + airportScalePower

    AirportRating(ratioToModelAirportPower, ratioToModelCountryPower, featurePower)
  }
}