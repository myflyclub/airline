...
class EasterCashAward(info: SantaClausInfo) extends EasterBunnyAward(info) {
  ...
  val CASH_AMOUNT = 20000000 * difficultyMultiplier
...
class EasterHqLoyaltyAward(info: SantaClausInfo) extends EasterBunnyAward(info) {
  ...
  override def applyAward(): Unit = {
    AirlineCache.getAirline(info.airline.id, fullLoad = true).foreach { airline =>
      airline.getHeadQuarter().foreach { hq =>
        val bonus = AirlineBonus(BonusType.EASTER_BUNNY, AirlineAppeal(loyalty = BONUS), Some(CycleSource.loadCycle() + Period.yearLength))
  ...
  override val description: String = {
    ...
    s"The Easter Bunny is hosting an egg hunt at your HQ for ${Period.yearLength} weeks! 🐣 Loyalty bonus +$BONUS at ${hq.displayText}!"
  }
}
...
class EasterAirportLoyaltyAward(info: SantaClausInfo) extends EasterBunnyAward(info) {
  ...
  override val description: String = s"The Easter Bunny stamped your airline logo on every egg being delivered to ${info.airport.displayText}! 🐰 Loyalty bonus +$BONUS at ${info.airport.displayText} for ${Period.yearLength} weeks!"
}
