...
object ChangeHistorySource {
  ...
  def loadLinkChangeByQueryString(queryString : String, parameters : List[Any]) : List[LinkChange]= {
    Using.resource(Meta.getConnection()) { connection =>
      Using.resource(connection.prepareStatement(queryString + " ORDER BY id DESC LIMIT 0, " + RESULT_SIZE)) { preparedStatement =>
  ...
  def saveLinkChanges(changes : List[LinkChange]) : Unit = {
    Using.resource(Meta.getConnection()) { connection =>
      ...
      Using.resource(connection.prepareStatement("INSERT INTO " + LINK_CHANGE_HISTORY_TABLE + "(link, price_economy, price_business, price_first, price_economy_delta, price_business_delta, price_first_delta, capacity_economy, capacity_business, capacity_first, capacity, capacity_economy_delta, capacity_business_delta, capacity_first_delta, capacity_delta, from_airport, to_airport, from_country, to_country, airline, alliance, frequency, flight_number, airplane_model, raw_quality, cycle) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")) { preparedStatement =>
